...
TEST_SUITE("Input and output operators tests") {
    ...
    TEST_CASE(">> operator with zero denominator") {
        std::stringstream ss_zerodenominator("3 0");

        Fraction frac1;
        CHECK_THROWS_AS(ss_zerodenominator >> frac1, std::runtime_error);

        ss_zerodenominator.str("6 8 3 0");
        Fraction frac2, frac3;
        CHECK_THROWS_AS(ss_zerodenominator >> frac2 >> frac3, std::runtime_error);
    }

    TEST_CASE(">> operator with zero numerator") {
        std::stringstream ss_validdenominator("0 4");

        Fraction frac;
        CHECK_NOTHROW(ss_validdenominator >> frac);
